...
- Verification: `npm run test -- tests/submissionAssets.test.ts`, `npm run test` (15 files, 99 tests), `npm run build`, `npm run test:e2e` (40 passed after serializing Playwright workers for stability), `npm audit --audit-level=moderate` (0 vulnerabilities), `git diff --check`, production deploy `dpl_Bbn8ySJ38Zd1LpkWHgA6JZtCVjg9`, and hosted smoke for `/api/mcp/status`, `/api/runtime-proof`, `/api/evaluate`, submission hub, Devpost pack, slide deck, walkthrough HEAD `content-length: 28485173`, desktop Judge mode, and mobile Judge mode.
