...
10. Review Student Impact Brief, Learning Impact Loop, Student Pilot Study Kit, Pilot Evidence Tracker, Learning Exit Ticket, Student Level Lens, Concept Mastery Check, and Student Reflection Workspace to see whether the student is ready to plan, reason, check understanding, log quality-gated anonymous pilot evidence, and explain answers themselves before writing.
11. Copy the Evidence Packet as a student-owned reasoning handoff with judge demo path, Custom Lab Triage, Pattern Archetype Coach, Pre-Lab Design Coach, AIYES rubric fit, AIYES values fit, Student Impact Brief, learning impact, pilot-study protocol, anonymous pilot evidence quality gate, Learning Exit Ticket, student-level lens, Concept Mastery Check prompts, student reflection drafts, guided flow, Grounding Audit, AI Evaluation Harness, Data Handling Ledger, sources, checks, safety notes, concept vocabulary, pattern evidence, repeat reliability, data table, next-trial plan, blanks, and next question.
...
17. Use MCP Integration Coach to validate Composio Source Scout, Composio Search source-audit, Scholar claim-check, Semantic Scholar reference-check, Composio Browser source-capture, DeepWiki public-source proof, and Canvas assignment-context routes plus Google Docs, Google Slides, Google Sheets, Google Drive, Google Classroom, Google Forms, Google Calendar, Gmail teacher-review draft, and Notion handoffs through the server dry-run bridge. The Source Scout receipt shows the verified read-only tool chain (`COMPOSIO_SEARCH_WEB`, `COMPOSIO_SEARCH_FETCH_URL_CONTENT`, `COMPOSIO_SEARCH_SCHOLAR`, `BROWSER_TOOL_CREATE_TASK`, `BROWSER_TOOL_WATCH_TASK`) plus July 15 live proof receipts: Composio Search found the official AIYES/Devpost source, while DeepWiki returned that `rushtanu14/ouija` needs indexing before live repo-architecture proof can be claimed. The Composio Sessions strategy separates read-only source verification and assignment-context sessions from later consent-gated export sessions without exposing credentials or sending student data during the public demo.
...
26. Open Judge Brief for a compact Track 1 proof surface: submission hub, live app, judge view, hosted deck/video/source links, AIYES submission checklist, judge demo path, official rubric fit, learning impact, student pilot protocol, student reflection drafts, progress portfolio, MCP readiness plan, AI pipeline, model strategy, architecture map, UX proof, evaluation, data-handling ethics, tests, and integrity constraints.
...
Official AIYES Rules Snapshot keeps the judge path tied to the current Devpost page: ages 13-18, students only, listed 2-5 team requirement with individual/team wording caveat, September 1 deadline, Track 1 artifacts, judging criteria, award bands, source link, July 16, 2026 verified date, and a snapshot that Devpost showed 83 participants at verification time.
...
AIYES Judge Q&A Prep makes the live judging defense explicit: likely questions about problem relevance, AI depth, integrity, UX, validation, and remaining claims each point to proof surfaces in the app, with a boundary against promising a Gold result, fake pilot data, or live connector access before credentials and consent exist.
...
MCP Integration Coach validates a practical Composio path for the same student-owned evidence: Source Scout first shows a read-only Composio discovery/fetch/scholar/browser path for grounding expected results, then Ouija can dry-run a source-audit search through Composio Search, run a Scholar claim check against the expected pattern, run a Semantic Scholar reference check for structured paper metadata, capture a dynamic public source page through Composio Browser Tool, audit public source proof through DeepWiki MCP, import Canvas assignment prompt/rubric context in read-only mode, create a Google Docs evidence packet, draft an editable Google Slides submission deck from Markdown, append table rows and pilot evidence export counts to Google Sheets, save a portfolio archive to Google Drive, draft a Google Classroom pre-lab checkpoint, create a Google Forms readiness check that can include pilot-evidence prompts and metrics, schedule a Google Calendar next-trial reminder, create an unsent Gmail teacher-review draft, or create a Notion learning record with student-authored reflection drafts. The July 15 Source Proof Receipt makes the external state explicit: Composio Search verified the official AIYES/Devpost source path, and DeepWiki MCP returned that the public repo must be indexed before live DeepWiki architecture proof is claimed. The Composio Sessions strategy now makes the route judge-legible: one read-only source verification bundle covers Search, Scholar, Semantic Scholar, Browser, and DeepWiki; a separate read-only Canvas assignment-context bundle can import lab prompts and rubrics; later Docs, Slides, Sheets, Drive, Classroom, Forms, Calendar, Gmail draft, and Notion exports stay blocked behind destination review and consent. The readiness matrix shows required auth config env vars where needed, allowed tools, least-privilege scopes, data shared, consent gates, dry-run checks, and a scoped Composio session ticket path. The public app now includes `/api/mcp/status`, `/api/mcp/export`, and `/api/mcp/session` for server-side validation; live connector execution still requires `COMPOSIO_API_KEY`, `COMPOSIO_SESSION_USER_ID`, allowed tools, `COMPOSIO_LIVE_EXPORTS=true`, `MCP_SESSION_AUTH_TOKEN`, connector auth config IDs where the toolkit requires them, and student or teacher consent before source audit, Scholar check, Semantic Scholar reference check, Browser source capture, DeepWiki source proof, Canvas assignment import, Google Slides draft creation, Gmail draft creation, or export.
