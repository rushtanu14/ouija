...
| `MCP_SESSION_AUTH_TOKEN` | No | Server-only bearer token required before `/api/mcp/session` can create a live Composio Tool Router session. Dry-runs still work without it. | long random token |
