...
- Added MCP Integration Coach plus `/api/mcp/status` and `/api/mcp/export` so students and judges can validate a credentials-safe Composio handoff to Google Docs, Google Sheets, Google Drive, Google Classroom, Google Forms, Google Calendar, and Notion, with a visible readiness matrix and consent-gated server dry-run.
