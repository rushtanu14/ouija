...
- Added MCP Integration Coach plus `/api/mcp/status`, `/api/mcp/export`, and `/api/mcp/session` so students and judges can validate a credentials-safe Composio handoff to Google Docs, Google Sheets, Google Drive, Google Classroom, Google Forms, Google Calendar, and Notion, with a visible readiness matrix, consent-gated server dry-run, and scoped session-ticket path.
