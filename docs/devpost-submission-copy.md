...
- Added MCP Integration Coach so students and judges can preview a credentials-safe Composio handoff to Google Docs, Google Sheets, Google Drive, Google Classroom, Google Forms, and Notion, with a visible readiness matrix.
...
Submit the hosted deck/video package on Devpost, handle the live page's listed 2-5 member team requirement, optionally enable the server-side Composio MCP bridge and a real OpenAI web-search-enriched demo, and keep expanding middle/high school experiment templates and adaptive next-trial profiles after the eight-demo foundation.
