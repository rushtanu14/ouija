...
`ffprobe` confirmed 1440x900 video, 28,485,173 bytes, duration 277.84 seconds, and SHA-256 `fdc7f98e8580caf8ed3767a5b46d5a922a52e08732874a6c2ad2b545ec70b4ab`. Local public package proof after sync is submission hub size `9352`, Devpost pack size `14039`, walkthrough size `28485173`, slide deck size `23277`, desktop screenshot size `4141522`, mobile screenshot size `2629786`, custom-triage screenshot size `4012572`, and warning-state screenshot size `4208588`.

Production deployment `dpl_Bbn8ySJ38Zd1LpkWHgA6JZtCVjg9` is aliased to `https://ouija-olive.vercel.app` and reflects the July 17 / 84-participant refresh plus the Pilot Protocol hardening. Hosted checks returned HTTP 200 for `/api/evaluate` with score `100`, `/api/mcp/status` with `server_dry_run` and Composio Search proof, `/api/runtime-proof` with `fallback_ready` and `9/9`, `/submission/` size `9352`, `/submission/devpost-pack.html` size `14039`, and `/submission/slide-deck.html` size `23277` with the 84-participant rules snapshot, Source Proof language, Pilot Protocol language, DeepWiki indexing boundary, 4:38 video demo label, and current walkthrough hash. Hosted walkthrough HEAD returned `content-length: 28485173`. Hosted desktop/mobile browser smoke confirmed Reaction Rate Judge mode, Pilot Protocol, DeepWiki public repo check with `Needs indexing`, 84 participants visible, no horizontal overflow, and zero console errors.
